 ...
 * @Annotate: 输入这页的内容
 * @LastEditTime: 2022-11-02 16:00:23
 * @LastEditors: wyy
 * @Description: 课程
 * @FilePath: /student_sys_vue/src/page/menu/dormitory.vue
  ...
  methods: {
    ...
    edit(row) {
      ...
      this.formLabelAlignEidt = {
        item: [{ name: "宿舍名称", inputContent: row.name, id: row.id }],
      };
    },
    closePopup() {
    ...
    submit(item) {
      ...
      addDormitoryApi({ name: item.item[0].inputContent }).then((res) => {
        // console.log(res);
        if (res.code == 200) {
          this.getListPage();
          this.closePopup();
    ...
    delContent(row) {
      ...
      this.$confirm("您确定删除此条数据？", "提示", {
        ...
        .then(() => {
          delDormitoryApi(row.id).then((res) => {
            if (res.code == 200) {
              this.getListPage();
              this.$message({
                message: res.data,
